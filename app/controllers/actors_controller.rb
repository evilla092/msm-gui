class ActorsController < ApplicationController
  ...
  def create
    ...
    @actor.image = params.fetch("query_img_url")
  ...
  def update
    ...
    if @actor.valid?
      @actor.save
      redirect_to("/actors/#{@actor.id}", { :notice => "Actors updated successfully."} )
    else
      redirect_to("/actors/#{@actor.id}", { :alert => "Actors failed to update successfully." })
    end
  end
